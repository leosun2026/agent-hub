...
Or double-click `start-hub.cmd` (auto-installs dependencies on first run)

访问: **http://127.0.0.1:3457**

---

## What's New in v0.2.0 / v0.2.0 新特性
...
│         Socket.IO 4 + sql.js                 │
├──────────────────────────────────────────────┤
│          SQLite (./hub.db created at runtime) │
└──────────────────────────────────────────────┘
```
...
SQLite database (`hub.db`) is auto-created at runtime in the project root. No manual setup required.
...
├── start-hub.cmd          # Windows batch startup script
├── start-hub.ps1          # PowerShell startup script
...
├── public/
│   ├── index.html         # Main SPA (splash + chat + tasks + office)
│   ├── agent-hub-logo.svg # App logo
│   ├── js/
│   │   ├── lang.js        # Internationalization (zh-CN / en)
│   │   ├── chat.js        # Chat + member management logic
│   │   └── pixel-office.js # Pixel office animation
│   └── office-assets/     # Pixel art sprites and assets
```
...
|---------|------|---------|
| v0.2.0 | 2026-07-01 | Dynamic member management, add/remove/edit agents, group_permissions, enhanced @mention, agent inter-chat, companion list, i18n bilingual |
