...
function renderTaskDropdown() {
  ...
  for (var i = 0; i < tasks.length; i++) {
    var task = tasks[i];
    var activeClass = task.id === currentTaskId ? " active" : "";
    html += '<div class="task-option' + activeClass + '" data-task-id="' + escAttr(task.id) + '" onclick="switchTask(this)">';
...
function loadHistory(taskId) {
  ...
  fetch(url)
    ...
    .then(function(msgs) {
      ...
      hintDiv.className = "chat-msg system";
      var now = new Date();
      var ds = now.getFullYear() + "-" + String(now.getMonth()+1).padStart(2,"0") + "-" + String(now.getDate()).padStart(2,"0");
      hintDiv.setAttribute("data-date", ds);
      hintDiv.innerHTML = "<div class=\"msg-text\">💡 <a href=\"/guide\" target=\"_blank\" style=\"color:var(--accent-primary);text-decoration:underline;\">" + t("msg_cmd_hint") + "</a></div>";
...
function sendMessage() {
  ...
  if (!currentTaskId) {
    var title = content.length > 5 ? content.substring(0, 5) + "..." : content;
    fetch("/api/tasks", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ title: title, participants: [] })
    }).then(function(r) { if (!r.ok) throw new Error("create failed"); return r.json(); })
    .then(function(task) {
      currentTaskId = task.id;
      tasks.push(task);
      try { localStorage.setItem("agentHubLastTaskId", task.id); } catch(e) {}
      selectTask(task.id);
      doSend(task.id);
    }).catch(function(e) {
      console.error("Auto-create task failed:", e);
      doSend(null);
    });
  } else {
    doSend(currentTaskId);
  }
}
...
function renderHistoryList(msgs) {
      ...
      keysToShow.forEach(function(key) {
        ...
        var month = t("history_date_format").replace("{m}", g.date.getMonth() + 1).replace("{d}", g.date.getDate());
        day.style.cursor = "pointer";
        day.setAttribute("data-date-key", key);
        day.onclick = function() { scrollToDate(key); };
        day.innerHTML =
          "<span>" + month + "</span>" +
          '<span class="day-count">' + g.count + t("history_msg_count") + '</span>' +
          '<div class="day-actions">' +
          '<button class="day-export-btn" data-i18n-title="msg_export" title="Export">↗</button>' +
          '<button class="day-delete-btn danger" data-i18n-title="msg_delete" title="Delete">✕</button>' +
          "</div>";
      ...
      if (!showAll && keys.length > 3) {
        moreBtn.innerHTML = '<span style="cursor:pointer;color:var(--accent-primary);font-size:12px;">\u25bc ' + t("history_more") + '</span>';
        ...
        moreBtn.innerHTML = '<span style="cursor:pointer;color:var(--accent-primary);font-size:12px;">\u25b2 ' + t("history_collapse") + '</span>';
...
function exportMonthChat(dateKey) {
  ...
  var label = t("history_date_format").replace("{m}", parseInt(parts[1])).replace("{d}", parseInt(parts[2]));
  var text = t("export_chat_title").replace("{date}", label) + "\n";
  text += "=".repeat(50) + "\n\n";
...
function exportChat() {
  ...
  if (!content) { alert(t("msg_export_empty")); return; }
  var now = new Date();
  ...
  fetch("/api/export", {
    ...
    if (d.success) { alert(t("msg_export_ok").replace("{path}", d.path)); }
    else { alert(t("msg_export_fail").replace("{error}", d.error || t("msg_unknown_error"))); }
  }).catch(function(){
      ...
      el.addEventListener("click", function() {
        ...
        if (cmd === "/stop") {
          if (typeof socket !== "undefined" && socket) {
            ...
            if (cm) {
              var div = document.createElement("div");
              div.className = "chat-msg system";
              div.innerHTML = "<div class=\"msg-text\">"+t("msg_stop_sent")+"</div>";
              cm.appendChild(div);
        ...
        if (cmd === "/help") {
          ...
          if (cm) {
            var div = document.createElement("div");
            div.className = "chat-msg system";
            div.innerHTML = "<div class=\"msg-text\">"+t("cmd_help_title")+"<br>/help - "+t("cmd_desc_help")+"<br>/stop - "+t("cmd_desc_stop")+"<br>/new - "+t("cmd_desc_new")+"<br>/save - "+t("cmd_desc_save")+"<br>/search - "+t("cmd_desc_search")+"<br>/Battle - " + t("cmd_desc_battle") + "</div>";
            cm.appendChild(div);
            cm.scrollTop = cm.scrollHeight;
          }
          return;
...
function doSearch() {
  ...
  results.innerHTML = "<div style=\"color:var(--text-muted);\">"+t("search_loading")+"</div>";
  fetch("/api/messages?room_id=room-general&limit=500")
    ...
    .then(function(msgs) {
      ...
      if (matches.length === 0) {
        results.innerHTML = "<div style=\"color:var(--text-muted);padding:8px;\">"+t("search_no_result").replace("{keyword}", keyword)+"</div>";
        return;
      }
